jQuery(document).ready(function($){
	...
	var faqsSections = $('.faq-group'),
		...
		closeFaqsContainer = $('.faq-close-panel');
	...
	$('body').bind('click touchstart', function(event){
		if( $(event.target).is('body.faq-overlay') || $(event.target).is('.faq-close-panel')) { 
			closePanel(event);
		}
	});
	...
	function updateCategoryPosition() {
		var top = $('.faq').offset().top,
			height = jQuery('.faq').height() - jQuery('.faq-categories').height(),
			margin = 20;
